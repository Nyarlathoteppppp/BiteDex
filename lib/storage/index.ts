...
import type { DailyLog, DailyLogsByDate, DexByFoodName, FoodCard } from "@/types"
import { buildDailyLog, mergeDexItem } from "@/lib/nutrition"

const dailyLogsKey = "bitedex.dailyLogs.v1"
const dexKey = "bitedex.dex.v1"
...
export function deleteFoodCard(foodId: string, date: string): DailyLog {
  ...
  const nextLog = buildDailyLog(
    date,
    currentLog.foods.filter((f) => f.id !== foodId),
  )
...
function rebuildDexFromLogs(logs: DailyLogsByDate): void {
